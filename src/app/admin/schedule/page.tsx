...
function WorkCompletionModal({
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setIsCompleted(true)
        // Create a simple receipt from the response
        const data = await res.json()
        if (data.booking) {
          const workDurationMinutes = data.booking.workDurationMinutes || Math.floor(currentDuration / 60)
          setReceipt({
            ...
            workStartTime: booking.workStartedAt || '',
            workEndTime: new Date().toISOString(),
            workDurationMinutes,
            workDurationFormatted: formatDurationText(workDurationMinutes),
            basePrice: booking.totalQuote,
            totalAmount: booking.totalQuote,
            depositAmount: null,
            depositPaid: false,
            balanceDue: booking.totalQuote,
          })
        }
      }
    } catch (error) {
  ...
  const handleCopyLink = () => {
    const text = `Work completed for ${booking.clientName} at ${booking.propertyAddress}. Duration: ${formatLiveDuration()}`
              ...
              {!isCompleted ? (
                ...
                <>
                  ...
                  <div className="border-t border-gold/10 pt-5">
                    <p className="text-sm text-cream-muted mb-3">Share Report</p>
  ...
  const showStatusConfirmation = (
    ...
    if (isGoingBack) {
      ...
      switch (newStatus) {
        ...
        case 'completed':
          title = 'Complete Work?'
          message = 'This will stop the timer and mark this job as completed.'
          variant = 'success'
          break
        default:
          title = 'Change Status?'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Button
          variant="secondary"
          onClick={fetchScheduledBookings}
          className="text-sm"
        >
